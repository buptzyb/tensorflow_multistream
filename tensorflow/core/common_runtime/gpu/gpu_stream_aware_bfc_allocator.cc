...
static const bool bfc_custom_log = [] {
  bool log;
  TF_CHECK_OK(tsl::ReadBoolFromEnvVar("TF_BFC_CUSTOM_LOG",
...
GPUStreamAwareBFCAllocator::GPUStreamAwareBFCAllocator(
    ...
    : BFCAllocator(std::move(sub_allocator), total_memory, name, opts) {}
...
bool GPUStreamAwareBFCAllocator::Extend(size_t alignment,
  ...
  static const int64 extend_limit_mb = [] {
    int64 limit;
    TF_CHECK_OK(tsl::ReadInt64FromEnvVar("TF_BFC_EXTEND_LIMIT_MB",
                                         /*default_val=*/-1, &limit));
    return limit;
  }();
...
void* GPUStreamAwareBFCWrapperAllocator::AllocateRaw(
  ...
  if (allocation_attr.stream_to_allocate == -1) {
    auto temp = AllocationAttributes(allocation_attr.retry_on_failure,
                                     allocation_attr.allocation_will_be_logged,
                                     allocation_attr.freed_by_func, stream_id_);
    return stream_aware_bfc_->AllocateRaw(unused_alignment, num_bytes, temp);
  }
